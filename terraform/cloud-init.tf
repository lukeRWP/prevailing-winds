...
resource "proxmox_virtual_environment_file" "cloud_init_base" {
  ...
  source_raw {
    ...
    file_name = "imp-cloud-init-base.yml"
  }
}

locals {
  cloud_init_snippet_id = var.manage_cluster_resources ? proxmox_virtual_environment_file.cloud_init_base[0].id : "${var.cloud_init_datastore}:snippets/imp-cloud-init-base.yml"
}
